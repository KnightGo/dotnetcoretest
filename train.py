...
weight=200
height=60
trainPath='/home/VerificationDemo/data/train'
testPath='/home/VerificationDemo/data/test'
if not os.path.exists('/home/VerificationDemo/checkpoints'):
    os.mkdir('/home/VerificationDemo/checkpoints')
...
def train():
    ...
    train_dataset = CaptchaData(trainPath, transform=transforms)
    train_data_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=0, 
                             shuffle=True, drop_last=True)
    test_data = CaptchaData(testPath, transform=transforms)
...
#            print('freezing layer:', param[0])
    ...
    for epoch in range(max_epoch):
        ...
        for img, target in test_data_loader:
            ...
            acc = calculat_acc(output, target)
            acc_history.append(float(acc))
            loss_history2.append(float(loss))
        print('test_loss: {:.4}|test_acc: {:.4}'.format(
                torch.mean(torch.Tensor(loss_history2)),
                torch.mean(torch.Tensor(acc_history)),
                ))
